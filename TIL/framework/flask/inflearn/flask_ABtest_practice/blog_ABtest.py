...
import os #보안 로그인할때 많이 사용하는 oauth2 ex;google,kakao 등 http 에서 테스트하기 위해서 사용
...
CORS(app)
app.secure.key = 'song_server' #보안 정책을 더 높일려면 서버를 킬때마다 갱신해주는 방법이 있다. 하지만 지금은 그렇게 해버리면 세션값들이 다 날아가기 때문에 고정값으로 하였다.
